...
*/
export function Lamborghini({wheel,paint}) {
  const { scene, nodes, materials } = useGLTF('/lambo.glb')
  const [pa ,setPa] = useState(paint)
  const [wh ,setWh] = useState(wheel)
  useEffect(()=>{
    setPa(paint)
    setWh(wheel)
    console.log(paint,wheel,'lambo')
  },[paint,wheel])
  useMemo(() => {
    ...
    applyProps(materials.TiresGum, { metalness: 0, roughness: 0.4, color: wh })
    ...
    applyProps(materials.LightsFrontLed, { color:"yellow",emissiveIntensity: 3, toneMapped: false })
  ...
  }, [nodes, materials,paint,wheel,pa,wh])
  return <primitive object={scene} rotation={[0, Math.PI / 1.5, 0]} scale={0.015} />
}
